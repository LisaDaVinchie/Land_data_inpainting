...
def main():
    ...
    mean = dataset[dataset_keys[-2]]
    std = dataset[dataset_keys[-1]]
    
    print(f"Dataset mean: {mean}, std: {std}", flush=True)
...
class TrainModel:
    def __init__(self, model, loss_function, optimizer, mean, std, clip_value = 5.0, lr_scheduler = None, save_every = 1):
    ...
    def denormalize(self, images: th.Tensor):
        """Denormalize the images tensor.

        Args:
            images (th.Tensor): images tensor

        Returns:
            th.Tensor: denormalized images tensor
        """
    ...
    def _compute_loss(self, images, masks, nan_masks):
        ...
        loss = self.loss_function(output[:, 0], self.denormalize(images[:, 4]), self.validation_mask(masks[:, 4], nan_masks[:, 4]))
        return loss
