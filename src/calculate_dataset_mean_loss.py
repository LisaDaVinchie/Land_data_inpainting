...
def main():
    ...
    for i in range(0, total_samples, batch_size):
        ...
        batch_mean = th.nanmean(batch_known, dim=1, keepdim=True)
        batch_mean = th.nan_to_num(batch_mean, nan=-300.0)
        batch_mean = th.where(batch_masks[:, c:c+1, :, :], batch_images[:, c:c+1, :, :], batch_mean)
        
        # Calculate validation mask and loss for this batch
        batch_val_mask = ~(~batch_masks & batch_nan_mask)
        batch_loss = loss_func(batch_mean, batch_images[:, c:c+1, :, :], 
                             batch_val_mask[:, c:c+1, :, :]).item()
        batch_valid_pixels = (~batch_val_mask[:, c, :, :]).float().sum().item()
        
        total_loss += batch_loss
        total_valid_pixels += batch_valid_pixels
        
        # Clean up batch variables
        del batch_images, batch_masks, batch_nan_mask, batch_known, batch_mean, batch_val_mask
        gc.collect()
    
    # Final calculation
    normalized_loss = total_loss / total_valid_pixels
    print(f"Final loss: {normalized_loss}", flush=True)
    print(f"Total valid pixels: {total_valid_pixels}", flush=True)
    
    th.save(th.tensor(normalized_loss), dataset_path.parent / f"mean_loss_{dataset_idx}.pt")
