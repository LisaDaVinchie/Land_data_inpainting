...
from losses1 import get_loss_function
...
class TrainModel:
    ...
    def train_step(self, dataloader, backpropagate=True):
        ...
        for (images, masks, nan_masks) in dataloader:
            ...
            n_images += self.calculate_valid_pixels(masks[:, self.current_day_channel],
                                                          nan_masks[:, self.current_day_channel])

        return epoch_loss / (n_images + 1e-8)  # Avoid division by zero
