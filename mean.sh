...
notify_telegram() {
    ...
    -d chat_id=${chat_id} \
    -d text="Your Land_data_inpainting slurm loss calculation (Job ID: $SLURM_JOB_ID) has completed with status: $status"
}
...
make loss || { echo "Loss calculation failed"; exit 1; }
...
echo "Loss calculation completed successfully"
